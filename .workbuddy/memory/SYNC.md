...
## 十二、同步铁律（2026-08-20 血泪教训后新增）

**GitHub main 仓库是唯一事实源（source of truth）。线上部署必须与仓库一致。**

1. **改完必 push**：任何一端修改代码后，必须 `git add -A && git commit && git push` 到 GitHub main，否则另一端拿不到，部署也会互相覆盖。
2. **开工前必 pull**：动手改任何文件前，先 `git pull --rebase origin main`（或 fetch 后检查）。绝对不要在本地的旧版本上工作。
3. **部署前必确认**：部署到 Cloudflare Pages 前，先确认本地 == 仓库 == 线上（curl 线上首页对比关键内容），再 `wrangler pages deploy`。
4. **绝不覆盖对方工作**：如果发现线上/仓库里有自己没见过的新内容（新页面、新产品），先恢复/拉取再改，禁止直接覆盖。
5. **push 失败必须说**：GitHub 443 墙导致 push 失败时，必须通过总线/邮件明确告知另一端"我没 push 成功"，绝不能装作已同步。
6. **Pages 手动部署**：本项目 Cloudflare Pages 非 git 集成，部署动作（wrangler pages deploy）由认领任务的一端执行，执行前 git pull 确保拿到最新。
